...
class App {
  ...
  onSelect = () => {
    if (!this.modelCloned && window.sunflower) {
      ...
      if (shadowMesh) {
        ...
        alert('Shadow mesh not found.');
      }

      // Set the flag to true to prevent further cloning
      this.modelCloned = true;

      // Hide the reticle after the model is cloned
      if (this.reticle) {
        this.reticle.hide();
      } else {
        alert('Reticle is not found.');
      }
    }
  }
